...
5. Audit the plan with `plan-auditor` on `cursor-grok-4.5-fast` when the fix spans more than two files or touches a public API. Otherwise `n/a: scoped local fix`.
6. Implement via `generalPurpose` on `cursor-grok-4.5-fast`. Prefer a failing repro commit or staged check before the fix so history tells the story.
